...
for k, d in dSignals.items():
    for dicIntern in d["data"]["data-schemas"]:
        lista= dicService.setdefault(dicIntern["alias"], [])
        for key, valores in dicIntern["values"].items():
            if valores["human-name"] not in dicSubservice:
                id +=1
                dicSubservice[valores["human-name"]]=id

            lista.append(Subservice(dicSubservice[valores["human-name"]], valores["human-name"]))
print(dicService.keys())
print(dicSubservice)
uploadMaster=True
if uploadMaster:
    listService = []
    index=100
    for schema, listSignals in dicService.items():
        listService.append(Service(index, "signal_monitoring", schema, [vars(ob) for ob in listSignals]))
        index += 1
...
dfSignals= df[df["event_general"] == "selectSignals_change"]
print(dfSignals[["user_id", "unifiedEvent","event_general"]].groupby(["user_id", "unifiedEvent"]).count())
